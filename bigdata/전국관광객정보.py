...
def getTourPointData(item,yyyymm,jsonResult):
    addCd = 0 if 'addrCd' not in item.keys() else item['addrCd']
    ...
    jsonResult.append({'yyyymm':yyyymm,'addrCd':addCd,'gungu':gungu,'sido':sido,'resNm':resNm,'rnum':rnum,'ForNum':ForNum,'NatNum':NatNum})
...
def main():
    ...
    print('%s_관광지입장정보_%d_%d.json'%(sido,nStartYear,nEndYear-1))
