...
def dcinside():
    if len(sys.argv) == 2:
        ...
        save_point = "./"
        now = time.localtime()
        pre_time = str(now.tm_year) + str(now.tm_mon) + str(now.tm_mday) + str(now.tm_hour) + str(now.tm_min) + str(
            now.tm_sec)
        save_time = str(now.tm_year) + "년" + str(now.tm_mon) + "월" + str(now.tm_mday) + "일" + str(
