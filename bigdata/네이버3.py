...
def count(data):
    ...
    for i in range(len(data)):
        if len(data[i]['org_link'].split('/')) > 2 :
            link.append(data[i]['org_link'].split('/')[0]+'//'+data[i]['org_link'].split('/')[2])
    ...
    link2 = sorted(link2,key=lambda link_in: int(link_in[1]),reverse= True)
    ...
    for i in link2:
        dd+=int(i[1])
    print("출처 없는 기사수:%s"%str(len(data)-dd))
